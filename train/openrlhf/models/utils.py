...
def process_sequences(sequences: torch.Tensor, input_len, eos_token_id, pad_token_id):
    ...
    last_token_indices = seq_length - attention_mask.long().fliplr().argmax(dim=1, keepdim=True) - 1
    mask = torch.arange(seq_length).unsqueeze(0).expand(sequences.size(0), -1).to(device=sequences.device)
    attention_mask = (mask >= first_token_indices) & (mask <= last_token_indices).to(dtype=torch.long)

    # in RL, state_i (current token) + action_i (next token) -> state_i+1 (next token)
    action_mask = attention_mask[:, input_len - 1 : -1]
    action_mask = action_mask.to(bool)
