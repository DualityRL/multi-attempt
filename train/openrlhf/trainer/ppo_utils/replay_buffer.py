...
def remove_padding_in_sequences(items):
    for item in items:
        seq, base_act_log_prob, act_log_prob, value, ret, adv, att_mask, act_mask, sys2_mask = (
            item.sequences,
        ...
        (
            ...
            act_mask[:right_pad],
            sys2_mask[:right_pad] if item.sys2_mask is not None else None,            
        )
    return items
...
class NaiveReplayBuffer(ABC):
    ...
    def normalize(self, attribute: str, strategy) -> None:
        ...
        action_masks = []        
        for item in self:           
            mask = item.action_mask
            if strategy.args.multi_attempt:
                mask = torch.logical_and(mask, torch.logical_not(item.sys2_mask))
            action_masks.append(mask)

            new_item = getattr(item, attribute)           
            if strategy.args.multi_attempt:
                new_item = new_item * mask
