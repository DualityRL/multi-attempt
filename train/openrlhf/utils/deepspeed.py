...
class DeepspeedStrategy(ABC):
    ...
    def setup_distributed(self, timeout=timedelta(minutes=120)) -> None:
    ...
    def offload_states(self, model, *args, **kwargs):
        assert isinstance(model, deepspeed.DeepSpeedEngine), f"model should be DeepSpeedEngine instead of {type(model)}"
        model.offload_states(*args, **kwargs)

    def reload_states(self, model):
        assert isinstance(model, deepspeed.DeepSpeedEngine), f"model should be DeepSpeedEngine instead of {type(model)}"
        model.reload_states()
